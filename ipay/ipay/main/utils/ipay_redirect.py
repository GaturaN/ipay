...
# per invoice/request by the access checks below.
ALL_OPERATOR_ROLES = OPERATOR_ROLES | {"iPay Collector", "iPay Sales"}
# Bundling several invoices into one prompt: operators and sales members (who chase a
# customer's whole balance). Field collectors stay one invoice at a time.
BUNDLER_ROLES = OPERATOR_ROLES | {"iPay Sales"}
